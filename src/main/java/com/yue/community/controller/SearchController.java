...
public class SearchController implements CommunityConstant {
    ...
    public String search(String keyword, Page page, Model model){ // 因为为GET请求，keyword不能通过请求体来传，通过路径来传
        // 搜索帖子
        org.springframework.data.domain.Page<DiscussPost> searchResult =
                elasticsearchService.searchDiscussPost(keyword, page.getCurrent() - 1, page.getLimit()); // page.getCurrent当前页从1开始，search要求从0开始，所以-1
        // 聚合数据
        List<Map<String, Object>> discussPosts = new ArrayList<>();
        if (searchResult != null){
            for (DiscussPost post : searchResult){
                Map<String, Object> map = new HashMap<>(); // 每次遍历，实例化一个map，聚合数据
                // 存帖子
                map.put("post", post);
                // 存作者
